...
async function saveSettings() {
  ...
  try { const parsed = JSON.parse(settings.customRules); if (!Array.isArray(parsed)) throw new Error('顶层必须是数组'); }
  catch (e) { document.getElementById('status').style.color = '#b91c1c'; document.getElementById('status').textContent = '规则 JSON 格式错误：' + e.message; return; }
  await chrome.storage.local.set(settings);
  document.getElementById('status').style.color = '#047857'; document.getElementById('status').textContent = '已保存。';
}
async function clearSettings() { await chrome.storage.local.set({ endpoint: DEFAULTS.endpoint, apiKey: '', model: DEFAULTS.model, extraPrompt: '' }); await loadSettings(); document.getElementById('status').textContent = '已清空 AI 设置。'; }
async function resetRules() { await chrome.storage.local.set({ customRules: DEFAULTS.customRules, groupByDomainFallback: true }); await loadSettings(); document.getElementById('status').textContent = '已恢复默认规则。'; }
